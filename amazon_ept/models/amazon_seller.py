...
class AmazonSellerEpt(models.Model):
    ...
    is_invoice_number_same_as_vcs_report = fields.Boolean(string="Is Invoice Number Same As VCS "
                                                                 "Report?")

    def deactivate_fba_warehouse(self):
        if self.amazon_program in ('pan_eu', 'cep'):
            location = self.amz_warehouse_ids.mapped('lot_stock_id')
            warehouses = tuple(self.amz_warehouse_ids.ids)
            for warehouse in self.amz_warehouse_ids:
                name = warehouse.name + 'archived' + (self.amazon_program)
                warehouse.name = name + str(time.time())[-2:]
                warehouse.code = warehouse.code + str(time.time())[-2:]
                warehouse.fulfillment_center_ids.unlink()
            unsellable_location = self.amz_warehouse_ids.mapped('unsellable_location_id')
            picking_types = self.env['stock.picking.type'].search([('warehouse_id', 'in', self.amz_warehouse_ids.ids)])
            rule_ids = self.env['stock.rule'].search([('warehouse_id', 'in', self.amz_warehouse_ids.ids)])
            rule_ids.write({'active': False})
            route_ids = self.amz_warehouse_ids.mapped('route_ids')
        ...
        if (self.amazon_program in ('efn', 'mci', 'efn+mci')) or (not self.amazon_program):
            warehouses = self.amz_warehouse_ids
            unsellable_location = self.amz_warehouse_ids.mapped('unsellable_location_id')
            for wh in warehouses:
                if not self.amazon_program:
                    name = wh.name + 'archived'
                else:
                    name = wh.name + 'archived' + (self.amazon_program)
    ...
    def fbm_auto_check_cancel_order_in_amazon(self, args={}):
        ...
        if seller_id:
            ...
            sale_order_obj.with_context({'is_auto_process': True}).cancel_amazon_fbm_pending_sale_orders(seller,\
                                                                        marketplaceids, seller.instance_ids.ids)
        return True
    ...
    def load_marketplace(self):
        ...
        marketplace_list = ['A2Q3Y263D00KWC', 'A2EUQ1WTGCTBG2', 'A1AM78C64UM0Y8', 'ATVPDKIKX0DER', 'A2VIGQ35RCS4UG',
                            ...
                            'A1VC38T7YXB528', 'A17E79C6D8DWNP', 'A1805IZSGTT6HS', 'A2NODRKZP88ZB9']
