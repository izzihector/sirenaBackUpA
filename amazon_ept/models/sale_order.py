...
class SaleOrder(models.Model):
    ...
    def get_data(self):
        ...
        data.update({
            'DisplayableOrderDateTime': self.amz_displayable_date_time,
            ...
            for order in orders:
                ...
                if not existing_order:
                    ...
                    comman_log_line_obj.amazon_create_order_log_line( \
                        message, model_id, False, amazon_order_ref, False, log_rec)
                    continue
                ...
                if existing_order and existing_order.state == 'draft':
                    ...
                    comman_log_line_obj.amazon_create_order_log_line( \
                        message, model_id, False, existing_order.name, False, log_rec)
        return True
        ...
        for wrapper_obj in list_of_wrapper:
            ...
            for order in orders:
                ...
                if existing_order:
                    ...
                    comman_log_line_obj.amazon_create_order_log_line( \
                        message, model_id, self.id, amazon_order_ref, \
                        False, log_rec)
                    continue
    ...
    def check_already_status_updated_in_amazon(self, seller, marketplaceids, instances):
        ...
        sales_orders = self.search([('warehouse_id', 'in', warehouse_ids), ('amz_order_reference', '!=', False),
    ...
    def create_or_find_amazon_product(self, order_details, product_details, instance,
        ...
        if not seller_sku:
            ...
            comman_log_line_obj.amazon_create_order_log_line(
                message, model_id, False, amazon_order_ref, seller_sku, log_book)
            ...
            if ctx.get('is_fba_order'):
                fulfillment_by = 'FBA'
            amazon_product = amz_product_obj.search_amazon_product(instance.id, seller_sku,
                                                                   fulfillment_by)
            if not amazon_product:
                ...
                if odoo_product:
                    ...
                    message = 'System have created new Odoo Product %s for %s instance' % (
                        seller_sku, instance.name)
                    comman_log_line_obj.amazon_create_order_log_line(
                        message, product_model_id, False, amazon_order_ref, seller_sku, log_book)
    ...
    def amz_create_order_lines(self, order, instance, order_details, dict_product_details):
        ...
        unit_price = float(order_details.get('ItemPrice').get('Amount').get('value')) / float(
            order_details.get('QuantityOrdered').get('value'))
        item_price = order.get_item_price(unit_price, float(
            order_details.get('ItemTax').get('Amount').get('value')))
    ...
    def get_amazon_fbm_shippig_vals_ept(self, instance, order, shipping_price, ship_tax):
        ...
        """
        if order.amz_seller_id.is_vcs_activated:
    ...
    def amz_create_sales_order(self, queue_order, log_book):
        ...
        queue_lines = queue_order.shipped_order_data_queue_lines.filtered(
        ...
        for line in queue_lines:
            ...
            if message:
                common_log_line_ept.amazon_create_order_log_line(message, model_id, False,
                                                                 amazon_order_ref, False, log_book)
                line.state = 'failed'
                continue
        ...
        for order_line_wrapper_obj in list_of_shipped_order_lines:
            ...
            if sales_order:
                ...
                if order_status == 'Shipped':
                    ...
                    common_log_line_ept.amazon_create_order_log_line(message, model_id, False,
                                                                     sales_order.amz_order_reference, False, log_book)
        ...
        for order_line in order_lines:
            ...
            if not sales_order or sales_order.amz_order_reference != amazon_order_ref:
                if order.get('BuyerEmail').get('value'):
                    ...
                    vat = ''
                    try:
                        vat = order.get('TaxRegistrationDetails', {}).get('member', {}).get(
                            'taxRegistrationId', {}).get('value', '')
                    except:
                        tax_registration_details = order.get('TaxRegistrationDetails', {}).get(
                            'member', {})
                        if isinstance(tax_registration_details, list):
                            for tax_registration_val in tax_registration_details:
                                if customer_vals.get('CountryCode') == tax_registration_val.get(
                                        'taxRegistrationAuthority', {}).get('country', {}).get( \
                                        'value', ''):
                                    vat = tax_registration_val.get('taxRegistrationId', {}).get( \
                                        'value', '')

                    if vat:
                        customer_vals.update({'vat-number': vat})

                    partner = fbm_sale_order_report_obj.get_partner(customer_vals, state_dict,
                                                                    country_dict, instance)
                if partner:
                    vals = self.prepare_amazon_sale_order_vals(instance, partner, \
                                                               order)
                    ...
                    updated_ordervals = self.prepare_updated_ordervals(
                        instance, amazon_order_ref, order)
        ...
        for amazon_order in amazon_orders:
            for picking in amazon_order.picking_ids:
                ...
                if not manage_multi_tracking_number_in_delivery_order:
                    ...
                    if amazon_order.warehouse_id.id != amazon_order.amz_instance_id.warehouse_id.id:
                        partner = amazon_order.warehouse_id.partner_id or amazon_order.warehouse_id.company_id.partner_id
    ...
    def amz_get_carrier_name_ept(picking):
        ...
        if picking.carrier_id and picking.carrier_id.amz_carrier_code:
            carrier_name = picking.carrier_id.amz_carrier_code.name
        else:
    ...
    def create_parcel_for_multi_tracking_number(self, parcel, message_id, order):
        ...
        message_information = ''
        ship_info = ""
        if order.warehouse_id.id != order.amz_instance_id.warehouse_id.id:
            partner = order.warehouse_id.partner_id or order.warehouse_id.company_id.partner_id
            ship_info = self.amz_get_ship_from_address_details(partner)
        if parcel.get('carrier_code'):
            carrier_information = '''<CarrierCode>%s</CarrierCode>''' % (parcel.get('carrier_code'))
        else:
