                        ...
                        if field.ttype in ['char', 'boolean', 'text', 'date', 'datetime', 'float', 'integer', 'selection', 'monetary', 'html']:
                            if model_rec.get(field.name):
                                ...
                                if object_data and query[0].lower() in str(object_data).casefold():
                                    ...
                                    search_result[self.env.user.company_id.name+'|' +
                                                  field.field_description+' : '+str(object_data)] = model_rec
                        ...
                        if field.ttype in ['one2many']:
                            if model_rec.get(field.name):
                                ...
                                for o2m_field in field_row.field_ids:
                                    o2m_field_list.append(
                                        o2m_field.field_id.name)

                                o2m_model_obj = self.env[related_o2m_model].search_read([('id', 'in', data_list), (
                                    'company_id', '=', self.env.user.company_id.name)], o2m_field_list, order='id')
                                ...
                                if o2m_model_obj:
                                    for o2m_model_rec in o2m_model_obj:
                                        for o2m_field_row in field_row.field_ids:
                                            ...
                                            if o2m_field.ttype in ['char', 'boolean', 'text', 'date', 'datetime', 'float', 'integer', 'selection', 'monetary', 'html']:
                                                if o2m_model_rec.get(o2m_field.name):
                                                    o2m_object_data = o2m_model_rec.get(
                                                        o2m_field.name)
                                                    if o2m_object_data and query[0].lower() in str(o2m_object_data).casefold():
                                                        ...
                                                        o2m_model_rec['id'] = model_rec.get(
                                                            'id')
                                                        search_result_model = str(
                                                            field.model_id.name)
                                                        search_result_record = model_rec.get(
                                                        ...
                                                        search_result[self.env.user.company_id.name+'|'+search_result_record +
                                                                      ' > '+o2m_field.field_description+' : '+str_object_data] = o2m_model_rec
                ...
                try:
                    ...
                    if company_id_field:
                        if single_company == 1:
                            ...
                            if count == 1:
                                model_obj = self.env[search_rec.model_id.model].search_read(
                                    ['|', ('company_id', '=', False), ('company_id', '=', company.id)], field_list, order='id')
                                count = 0
                            else:
                                model_obj = self.env[search_rec.model_id.model].search_read(
                                    [('company_id', '=', company.id)], field_list, order='id')
                            ...
                            for field_row in search_rec.global_field_ids:
                                ...
                                if field.ttype in ['char', 'boolean', 'text', 'date', 'datetime', 'float', 'integer', 'selection', 'monetary', 'html']:
                                    if model_rec.get(field.name):
                                        ...
                                        if object_data and query[0].lower() in str(object_data).casefold():
                                            model_rec['model'] = field.model_id.model
                                            model_rec['model_name'] = field.model_id.name
                                            search_result[company.name+'|'+field.field_description+' : '+str(
                                ...
                                if field.ttype in ['one2many']:
                                    if model_rec.get(field.name):
                                        ...
                                        o2m_model_obj = self.env[related_o2m_model].search_read(
                                            [('id', 'in', data_list), ('company_id', '=', company.id)], o2m_field_list, order='id')
